...
export const getTrending = async (req, res, next) => {
    try {
        ...
        const sortedData = [...data].sort((a, b) => b.raw_percent_change['24h'] - a.raw_percent_change['24h']);
        res.json({
            status: {
                timestamp: new Date(),
                error_code: 0,
 ...
 */
export const getByTimePeriod = async (req, res, next) => {
    try {
        ...
        const sortedData = [...data].sort((a, b) => b.raw_percent_change[period] -
            a.raw_percent_change[period]);
        res.json({
            status: {
                timestamp: new Date(),
                error_code: 0,
...
const applyFilters = (data, options) => {
    ...
    const { filter, sortBy = 'market_cap', sortPeriod = '24h', sortDirection = 'desc', search, } = options;
    // Apply search filter if provided
    if (search) {
        const searchLower = search.toLowerCase();
        filteredData = filteredData.filter((crypto) => crypto.name.toLowerCase().includes(searchLower) ||
    ...
    if (filter) {
        switch (filter) {
            case 'trending':
                filteredData.sort((a, b) => b.raw_percent_change['24h'] - a.raw_percent_change['24h']);
                break;
            case 'gainers':
            ...
            case '24h':
                ...
                filteredData.sort((a, b) => sortDirection === 'asc'
        ...
        switch (sortBy) {
            case 'price':
                filteredData.sort((a, b) => sortDirection === 'asc' ? a.price - b.price : b.price - a.price);
                break;
            case 'market_cap':
                filteredData.sort((a, b) => sortDirection === 'asc'
...
export const getMasterData = async (req, res, next) => {
    try {
        ...
        const sortPeriod = req.query.sortPeriod || '24h';
        const sortDirection = req.query.sortDirection?.toLowerCase() === 'asc' ? 'asc' : 'desc';
